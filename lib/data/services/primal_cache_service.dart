...
class PrimalCacheService {
  ...
  final Map<String, Completer<Map<String, int>>> _pendingRequests = {};
    ...
    try {
      ...
      if (data is List && data.length >= 2) {
        ...
        if (messageType == 'EVENT' && data.length >= 3) {
          ...
          if (kind == USER_FOLLOWER_COUNTS) {
            final completer = _pendingRequests.remove(subscriptionId);
  ...
  Future<Map<String, int>> fetchFollowerCounts(List<String> pubkeyHexes) async {
    ...
    _pendingRequests[subscriptionId] = completer;

    try {
      final ws = await _webSocketManager.getOrCreateConnection(primalCacheUrl);

      if (ws == null) {
        _pendingRequests.remove(subscriptionId);
        return {};
      }
