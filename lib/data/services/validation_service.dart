...
class ValidationService {
  ...
  Result<void> validateNpub(String npub) {
    if (npub.trim().isEmpty) {
      return Result.error('NPUB cannot be empty');
    }

    if (npub.startsWith('npub1')) {
      if (npub.length < 63) {
        return Result.error('NPUB is too short');
      }
      ...
      try {
        ...
        return Result.error('Invalid hex public key format');
      }
  ...
  Result<void> validatePublicKeyHex(String publicKey) {
    if (publicKey.trim().isEmpty) {
      return Result.error('Public key cannot be empty');
    }

    if (publicKey.length != 64) {
      return Result.error('Public key must be 64 characters long');
    }
    ...
    try {
      ...
      return Result.error('Public key must be valid hexadecimal');
    }
  }
  ...
  Result<void> validateProfileName(String name) {
    ...
    if (name.length > 50) {
      return Result.error('Name is too long (max 50 characters)');
    }
  ...
  Result<void> validateUrl(String url) {
    if (url.trim().isEmpty) {
      return Result.success(null); // Empty URL is valid
    }
  ...
  Result<void> validateNip05(String nip05) {
    if (nip05.trim().isEmpty) {
      return Result.success(null); // Empty NIP-05 is valid
    }
    ...
    if (!domain.contains('.')) {
      return Result.error('NIP-05 domain must be valid domain name');
    }
  ...
  Result<void> validateLightningAddress(String lud16) {
    if (lud16.trim().isEmpty) {
      return Result.success(null); // Empty LUD-16 is valid
    }
  ...
  Result<void> validateRelayUrl(String relayUrl) {
    if (relayUrl.trim().isEmpty) {
      return Result.error('Relay URL cannot be empty');
    }
    ...
    try {
      ...
      if (uri.host.isEmpty) {
        return Result.error('Invalid relay URL format');
      }
  ...
  Result<void> validateZapAmount(int amount) {
    if (amount <= 0) {
      return Result.error('Zap amount must be greater than 0');
    }

    if (amount > 100000000) {
      return Result.error('Zap amount is too large (max 1 BTC)');
    }
