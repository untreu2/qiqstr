...
class BatchProcessingService {
  ...
  BatchProcessingService({required NetworkService networkService})
  ...
  void addEventToBatch(Map<String, dynamic> eventData, {int priority = 2}) {
    ...
    _eventQueue.add(BatchItem(eventData, priority));
    
    if (_eventQueue.length >= _maxEventBatchSize) {
      _flushEventBatch();
    } else {
  ...
  void _flushEventBatch() {
    if (_eventQueue.isEmpty || _isProcessingEvents) return;
    
    _isProcessingEvents = true;
    _eventBatchTimer?.cancel();
    
    final batch = <Map<String, dynamic>>[];
    final batchSize = _eventQueue.length.clamp(1, _maxEventBatchSize);
  ...
  void addProfileToBatch(String npub, {int priority = 2}) {
    ...
    if (!_profileQueue.any((item) => item.data == npub)) {
      _profileQueue.add(BatchItem(npub, priority));
    }
    
    if (_profileQueue.length >= _maxBatchSize) {
      _flushProfileBatch();
    } else {
  ...
  void _flushProfileBatch() {
    if (_profileQueue.isEmpty || _isProcessingProfiles) return;
    
    _isProcessingProfiles = true;
    _profileBatchTimer?.cancel();
    
    final batch = <String>[];
    final batchSize = _profileQueue.length.clamp(1, _maxBatchSize);
  ...
  void addInteractionBatch(List<String> eventIds, {int priority = 2}) {
    ...
    _interactionQueue.add(BatchItem(eventIds, priority));
    
    if (_interactionQueue.length >= 3) {
  ...
  void _flushInteractionBatch() {
    if (_interactionQueue.isEmpty || _isProcessingInteractions) return;
